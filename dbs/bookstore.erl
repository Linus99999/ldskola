%%%-------------------------------------------------------------------
%%% @author Linus Östrén, David Rönn Jansson
%%%
%%% Created : 28 Jan 2014
%%%-------------------------------------------------------------------
...
server(State) -> 
	...
	{add, Book, Author, Price, Qty} ->
		...
		false -> StateNew = [{Book, Author,Price, Qty}|State],
			 ...
			 server(StateNew);
		_-> server(State) %% it makes sense to add quantity here but it's specified that we don't do anything.
	    end;
	{buy, Book, Pid} ->
	    case lists:keysearch(Book,1, State) of
		false -> io:fwrite("buy request recieved from ~p ~n Sorry ~p, we don't have ~p",[Pid,Pid, Book]),
			 server(State);
		{_,Tuple} -> case element(4,Tuple) of
				 0 -> io:fwrite("buy request recieved from ~p ~n Sorry ~p, ~p is sold out",[Pid,Pid, Book]),
				      server(State);
				 _ -> StateNew = lists:keyreplace(Book, 1, State,setelement(4,Tuple, element(4,Tuple) -1)),
				      io:fwrite("buy request recieved from ~p ~n Client ~p bought a copy of ~p~n", [Pid,Pid,Book]),
				      io:format("new inventory: "),
				      lists:map(fun(X) -> io:format("~p~n",[X]) end, StateNew),
				      server(StateNew)
			     end
	    end;
	close -> io:format("closed for today!~n"),
		   exit(normal);
	_ -> io:fwrite("What is this witch-craft?!~n"),
	     server(State)
